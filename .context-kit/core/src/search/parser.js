...
export class QueryParser {
            ...
            if (lastSlash > 0) {
                ...
                try {
                    ...
                    return {
                        type: 'regex',
                        value: pattern,
                        modifiers: flags ? { regexFlags: flags } : undefined
                    };
                }
                catch (error) {
    ...
    validate(parsedQuery) {
        ...
        if (parsedQuery.type === 'regex') {
            try {
                const flags = parsedQuery.modifiers?.regexFlags || '';
