...
export class StatementManager {
    ...
    maxStatements;
    db;
    constructor(db, maxStatements = 100) {
        this.db = db;
    ...
    get(name, sql) {
        ...
        try {
            ...
            throw new DatabaseError(`Failed to prepare statement '${name}': ${error instanceof Error ? error.message : 'Unknown error'}`, 'PREPARE_ERROR', error instanceof Error ? error : undefined);
        }
    }
    /**
 ...
 */
export function createCommonStatements(db, maxStatements) {
