...
export class ValidationUtils {
  ...
  static validateLogFilters(filters: LogFilters): ValidationError[] {
    ...
    if (filters.level && typeof filters.level === 'string' && !validLevels.includes(filters.level)) {
    ...
    if (Array.isArray(filters.level)) {
      ...
      if (invalidLevels.length > 0) {
        errors.push(this.createValidationError('level', invalidLevels, `Valid log levels: ${validLevels.join(', ')}`));
      }
    }

    if (filters.service) {
      if (Array.isArray(filters.service)) {
        if (filters.service.some(s => typeof s !== 'string')) {
          errors.push(this.createValidationError('service', filters.service, 'Array of strings'));
        }
      } else if (typeof filters.service !== 'string') {
        errors.push(this.createValidationError('service', filters.service, 'String or array of strings'));
      }
    }
   ...
   */
  private static createValidationError(field: string, value: any, expected: string): ValidationError {
    const error = new Error(`Validation failed for field '${field}': expected ${expected}, got ${typeof value === 'object' ? JSON.stringify(value) : value}`) as ValidationError;
   ...
   */
  static throwIfInvalid(errors: ValidationError[], context: string = 'Validation'): void {
    if (errors.length > 0) {
      const error = new Error(`${context} failed: ${errors.map(e => e.message).join('; ')}`) as ValidationError;
      error.code = 'VALIDATION_ERROR';
