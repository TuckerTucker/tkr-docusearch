...
class DocumentStructure:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "code_blocks": [
                {"page": c.page_num, "language": c.language, "lines": c.num_lines, "bbox": c.bbox}
                for c in self.code_blocks
            ],
            "formulas": [
                {"page": f.page_num, "latex": f.latex, "bbox": f.bbox} for f in self.formulas
            ],
            "summary": {
...
def validate_metadata_size(metadata: dict, max_size_kb: int = 50) -> None:
    ...
    if size_kb > max_size_kb:
        raise ValueError(f"Metadata size {size_kb:.1f}KB exceeds limit {max_size_kb}KB")
...
def validate_document_structure(structure: DocumentStructure) -> None:
    ...
    assert all(h.page_num > 0 for h in structure.headings), "Invalid page numbers in headings"
    assert all(h.text.strip() for h in structure.headings), "Empty heading text"

    # Check table consistency
    assert all(t.num_rows >= 0 for t in structure.tables), "Invalid row count in tables"
    assert all(t.num_cols >= 0 for t in structure.tables), "Invalid column count in tables"

    # Check picture consistency
    assert all(
        0 <= p.confidence <= 1 for p in structure.pictures
...
def validate_chunk_context(context: ChunkContext) -> None:
    ...
    if context.parent_heading_level is not None:
        assert (
            0 <= context.parent_heading_level <= 10
        ), f"Invalid heading level: {context.parent_heading_level}"

    # Check section path length
    if context.section_path:
        assert len(context.section_path) < 500, "Section path too long"

    # Check element type
    valid_types = ["text", "list_item", "table_cell", "caption", "code", "formula"]
    assert context.element_type in valid_types, f"Invalid element type: {context.element_type}"

    # Check page numbers
    assert all(p > 0 for p in context.page_nums), "Invalid page numbers"
