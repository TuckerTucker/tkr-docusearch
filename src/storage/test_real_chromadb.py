...
visual_results = chroma.search_visual(
    query_embedding=query_result['cls_token'],
    n_results=5
)

text_results = chroma.search_text(
    query_embedding=query_result['cls_token'],
    n_results=5
)
...
if visual_results:
    print(f"    Top visual result: {visual_results[0]['id']}, distance={visual_results[0]['distance']:.4f}")

if text_results:
    print(f"    Top text result: {text_results[0]['id']}, distance={text_results[0]['distance']:.4f}")
...
if text_results:
    ...
    scores = engine.score_multi_vector(
        ...
        document_embeddings=[top_full_emb]
    )
