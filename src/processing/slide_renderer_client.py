...
class SlideRendererClient:
    ...
    def render_slides(
        ...
        logger.info(
            f"Rendering slides: {Path(pptx_path).name} → {output_dir} @ {dpi} DPI"
        )
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    raise FileNotFoundError(
                        f"PPTX file not found by service: {pptx_path}"
                    )
                else:
                ...
                last_error = e
                logger.warning(
                    f"Rendering timeout on attempt {attempt}/{max_retries}: {e}"
                )
        ...
        raise ConnectionError(
            f"Failed to render slides after {max_retries} attempts: {last_error}"
        )
