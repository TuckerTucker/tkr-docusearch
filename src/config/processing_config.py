...
visual processing parameters, and ASR (speech recognition).
"""
...
class ProcessingConfig:
    ...
    def validate_file(self, filename: str, size_bytes: int) -> Tuple[bool, str]:
        ...
        if size_bytes > self.max_file_size_mb * 1024 * 1024:
            return False, f"File too large: {size_bytes} bytes (max {self.max_file_size_mb}MB)"
        return True, ""
    ...
    def supported_extensions_set(self) -> Set[str]:
        ...
        return {f".{fmt.value}" for fmt in FileFormat}
